...
d_optim = ai.Optimizer(critic.layers, optim_fn='Adam', lr=alpha, graph=D_graph)
...
m = 8   # batch size
k = 1   # number of critic updates per generator update
...
while epoch < 1:
    ...
    for _ in range(k):
        ...
        real_probs = critic.forward(real_images)
        ...
        fake_probs = critic.forward(fake_images)
