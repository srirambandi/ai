...
optim = ai.Optimizer(test.parameters(), optim_fn='Adam', lr=1e-3)
...
while it < 1:
    ...
    test.convt.K.data = np.ones((1, 1, 3, 3))
    ...
    print(res.data[0, :, :, 0])
    print(res.grad[0, :, :, 0])
    print(test.convt.K.grad[0, 0])
    print(test.convt.b.grad)
    print(X.grad[0, :, :, 0])

    # loss = L.loss(res[-1], None)
    # L.backward()

    # testing the library here....
    # print([X[...,i] for i in range(X.shape[-1])])
    # print('Conv2d kernel\n', [test.conv1.K.data[i,...] for i in range(test.conv1.K.data.shape[0])],'\n')
    # print('Conv2d bias\n', test.conv1.b.data, '\n')
